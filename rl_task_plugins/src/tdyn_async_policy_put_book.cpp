...
#include <rl_task_plugins/tdyn_async_policy_put_book.h>
...
namespace hiqp
{
  namespace tasks
  {
    ...
    TDynAsyncPolicyPutBook::TDynAsyncPolicyPutBook(
      std::shared_ptr<GeometricPrimitiveMap> geom_prim_map,
      std::shared_ptr<Visualizer> visualizer)
    : TaskDynamics(geom_prim_map, visualizer) {}


      int TDynAsyncPolicyPutBook::init(const std::vector<std::string>& parameters, 
      ...
      if (size != 4) {
        printHiqpWarning("TDynAsyncPolicyPutBook requires 4 parameters, got "
      ...
      act_sub_ = nh_.subscribe(action_topic_, 1, &TDynAsyncPolicyPutBook::handleActMessage, this);
      state_pub_ = nh_.advertise<rl_task_plugins::StateMsgWithCorners>(state_topic_, 1);

      return 0;
    }

    int TDynAsyncPolicyPutBook::update(const RobotStatePtr robot_state,
                                const TaskDefinitionPtr def) {

      update_lock_.lock();
      if (!initialized_) {
          ROS_ERROR("TDynAsyncPolicyPutBook not initialized!");
          update_lock_.unlock();
          return -1;
      }
      ...
      desired_stream.close();
        ...
        Eigen::MatrixXd book_corners = def->getBookCorners();
        ...
        if(d.toSec() >= 1.0 / publish_rate_ ) {
            rl_task_plugins::StateMsgWithCorners msg;
            msg.n_joints = q.rows();
            ...
            msg.book_corners = std::vector<double>(book_corners.data(),book_corners.data()+book_corners.size());
    ...
    int TDynAsyncPolicyPutBook::monitor() {
      return 0;
    }

    void TDynAsyncPolicyPutBook::handleActMessage(const rl_task_plugins::DesiredErrorDynamicsMsgConstPtr 
            &act_msg) {
        ...
        if(act_msg->e_ddot_star.size() != e_ddot_star_.rows()) {
            printHiqpWarning("TDynAsyncPolicyPutBook: mismatch between dimensions of eddot_star and dynamics provided in message");
        ...
        if(act_msg->e_ddot_star.size() != desired_dynamics_.rows()) {
            printHiqpWarning("TDynAsyncPolicyPutBook: mismatch between dimensions of desired and available errors, resizing");
    ...
    void TDynAsyncPolicyPutBook::publishStateMessage(const Eigen::VectorXd &error) {
...
PLUGINLIB_EXPORT_CLASS(hiqp::tasks::TDynAsyncPolicyPutBook,
 hiqp::TaskDynamics)
