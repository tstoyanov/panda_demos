...
#include <rl_task_plugins/tdef_rl_put_book.h>
...
namespace hiqp {
    namespace tasks {
        ...
        int TDefRLPutBook::init(const std::vector<std::string>& parameters,
            ...
            if (size != 15 ) {
                printHiqpWarning("'" + getTaskName() +
                                 "': TDefRLPutBook expects 15 parameters ");
                return -1;
            }
            ...
            // Initialization for book corners
            corner_points_.resize(n_bookcorners);
            pose_corners_.resize(n_bookcorners);
            book_corners = Eigen::MatrixXd::Zero(n_bookcorners, n_controls); // 4*3
            ...
            if((KDL::dot(normal1_,normal2_) > 1e-5) ||
                ...
                printHiqpWarning("'" + getTaskName() +
                                 "': Task not added, TDefRLPutBook expects orthogonal normal vectors ");
                return -2;
            }
            ...
            if (target_point_ == nullptr) {
                printHiqpWarning(
                        "In TDefRLPutBook::init(), couldn't find primitive with name "
                        "'" +
            ...
            for(unsigned int i=0; i<n_bookcorners; i++)
            {
                std::shared_ptr<GeometricPrimitiveMap> gpm = this->getGeometricPrimitiveMap();

                corner_points_.at(i) = gpm->getGeometricPrimitive<GeometricPoint>(parameters.at(11+i));
                if (corner_points_.at(i) == nullptr) {
                    printHiqpWarning(
                            "In TDefRLPutBook::init(), couldn't find primitive with name "
                            "'" +
                            parameters.at(11+i) + "'. Unable to create task!");
                    return -3;
                }

                gpm->addDependencyToPrimitive(parameters.at(11+i), this->getTaskName());
            }

            return 0;
        }

        int TDefRLPutBook::update(RobotStatePtr robot_state)
        {
            int retval = 0;
            ...
            if (retval != 0) {
                std::cerr << "In TDefRLPutBook::update : Can't solve position "
            ...
            if (retval != 0) {
                std::cerr << "In TDefRLPutBook::update : Can't solve jacobian "
                          << "of link '" << target_point_->getFrameId() << "'"
                          << " in the "
            ...
            if (retval != 0) {
                std::cerr << "In TDefRLPutBook::update : Can't solve jacobian derivative "
                          << "of link '" << target_point_->getFrameId() << "'"
                          << " in the "
            ...
            // corner points position
            for (unsigned int i=0; i<n_bookcorners; i++)
            {
                retval = fk_solver_pos_->JntToCart(robot_state->kdl_jnt_array_vel_.q, pose_corners_.at(i),
                                               corner_points_.at(i)->getFrameId());
                if (retval != 0) {
                    std::cerr << "In TDefRLPutBook::update : Can't solve position "
                            << "of link '" << corner_points_.at(i)->getFrameId() << "'"
                            << " in the "
                            << "KDL tree! KDL::TreeFkSolverPos_recursive::JntToCart return "
                            << "error code '" << retval << "'\n";
                    return -1;
                ...
                KDL::Vector p1__ = pose_corners_.at(i).M * corner_points_.at(i)->getPointKDL(); 
                KDL::Vector p1 = pose_corners_.at(i).p + p1__;

                book_corners(i,0) = dot(normal1_,p1);
                book_corners(i,1) = dot(normal2_,p1);
                book_corners(i,2) = dot(normal3_,p1);
            }
        ...
        int TDefRLPutBook::monitor() { return 0; }

    }  // namespace tasks

}  // namespace hiqp

PLUGINLIB_EXPORT_CLASS(hiqp::tasks::TDefRLPutBook,
                       hiqp::TaskDefinition)